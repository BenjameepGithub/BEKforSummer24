...
namespace Utilities.CustomAttributes {
    ...
    public class AssetPreviewIconDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            var sliderRect = new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight, position.width,
                EditorGUIUtility.singleLineHeight);
            _iconSize = EditorGUI.Slider(sliderRect, "Icon Size", _iconSize, 64, 128);

            var iconRect = new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight * 2, _iconSize,
                _iconSize);
